...
// Checks the three things the station depends on: every game's set is reproducible
// (two listeners computing the same set get identical moves), the faster chess
...
const chessSlots = [];
for(let s = 0; chessSlots.length < 3; s++) chessSlots.push(s * 7919);
let positions = 0;
for(const slot of chessSlots){
  for(const e of STATION.buildSlot(slot, 'chess')){
...
let compared = 0;
for(const e of STATION.buildSlot(chessSlots[0], 'chess').filter((e, i) => i % 6 === 0)){
...
// reproducibility and timing: every game, in the current set
const slot = STATION.slotIndexAt(Date.now());
for(const id of STATION.ORDER){
  const t0 = Date.now();
  const events = STATION.buildSlot(slot, id);
  const ms = Date.now() - t0;
  check(JSON.stringify(events) === JSON.stringify(STATION.buildSlot(slot, id)), `${id} set ${slot} not reproducible`);
